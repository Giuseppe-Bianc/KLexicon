...
class Lexer(inp: String) : LBase(inp) {
    private val KEYWORDS = arrayOf("if", "else", "while", "for", "return")
    ...
    /*inline fun <reified T> lex(): T = when (T::class) {
        List::class -> sLex().toList() as T
        Array<Token>::class -> lexArray() as T
